...
interface IconBtnProps {
  onClick: () => void;
  label: string;
  off?: boolean;        // true = toggled off (show strikethrough state)
...
function PillBtn({ onClick, label, variant, children }: PillBtnProps) {
  const styles = {
    ...
    ghost: 'bg-transparent text-white/50 hover:text-white border border-white/10 hover:border-white/20',
  };
  return (
    <button
      onClick={onClick}
      aria-label={label}
      className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-150 active:scale-95 ${styles[variant]}`}
    >
      {children}
      <span>{label}</span>
    </button>
      ...
      <IconBtn onClick={toggleCamera} label={isCameraOff ? 'Camera on' : 'Camera off'} off={isCameraOff}>
        {isCameraOff ? (
          ...
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            ...
            <rect x="1" y="5" width="15" height="14" rx="2" ry="2" stroke="currentColor" strokeWidth="1.8" fill="none"/>
          </svg>
        )}
      </IconBtn>
